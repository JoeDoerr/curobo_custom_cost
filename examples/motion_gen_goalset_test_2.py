...
# initial_test = False
# if initial_test:
#     result = motion_gen.plan_goalset(start_state, goal_pose, m_config, link_poses=None)
...
goalset_test = True
if goalset_test:
    result = motion_gen.plan_goalset(start_state, Pose(pos, quat), m_config, is_warmup=False)

    print("Goalset query result")
    print(result.success, result.status)


print(pos.shape, quat.shape)

for i, (p, q) in enumerate(zip(pos.squeeze(0), quat.squeeze(0))):
    # print(p, q)
    temp_ray_goal = Pose(
        p, q
    )
    ...
    print(single_result.success, single_result.status)